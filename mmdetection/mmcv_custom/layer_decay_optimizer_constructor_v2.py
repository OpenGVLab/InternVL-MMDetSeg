...
def get_num_layer_for_vit(var_name, num_max_layer):
    ...
    if var_name in ("backbone.cls_token", "backbone.mask_token",
        ...
        return layer_id + 1
    elif var_name.startswith("bbox_head.transformer.encoder.layers."):
        layer_id = int(var_name.split(".")[4])
        return num_max_layer - 1 - (12 - layer_id - 1) - 1
    elif var_name.startswith("bbox_head.transformer.decoder.layers."):
        layer_id = int(var_name.split(".")[4])
        return num_max_layer - 1 - (6 - layer_id - 1) - 1
    else:
        return num_max_layer - 1


@OPTIMIZER_BUILDERS.register_module()
class LayerDecayOptimizerConstructorV2(DefaultOptimizerConstructor):
