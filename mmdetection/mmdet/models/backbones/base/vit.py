...
from mmcv_custom import load_checkpoint as load_checkpoint
...
class WindowedAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
                 causal=False, use_flash_attn=False, window_size=14):
        super().__init__()
        self.num_heads = num_heads
        head_dim = dim // num_heads
        self.scale = head_dim ** -0.5
    ...
    def forward(self, x, H, W, key_padding_mask=None, need_weights=False):
        # cls, x = x[:, :1, :], x[:, 1:, :]
        B, N, C = x.shape
        ...
        def _flash_attn(x):
            ...
            context, _ = self.inner_attn(qkv, key_padding_mask=key_padding_mask, need_weights=need_weights,
                                         causal=self.causal)
...
class TIMMVisionTransformer(BaseModule):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, residual_indices=[], embed_dim=768,
                 ...
                 use_flash_attn=False, pretrained=None):
        """
        Args:
    ...
    def forward(self, x):
        x = self.forward_features(x.to(self.dtype))
        return x
